...
const SCALING_TRIGGER_MECHANICS = new Set([
  "slow-scaling",
  "kill-scaling",
  "distance-scaling",
  "attack-scaling",
...
for (const profile of profileCatalog.profiles ?? []) {
  ...
  if (!towerIds.has(id)) {
    errors.push(
      `${id}: towerId does not exist in towers.v2.json`,
    );
  }

  if (profileIds.has(id)) {
    errors.push(
      `${id}: duplicate tower profile`,
    );
  } else {
    profileIds.add(id);
  }
  ...
  if (profile.offense) {
    ...
    if (
      !DAMAGE_DELIVERIES.has(
        profile.offense.damageDelivery,
      )
    ) {
      errors.push(
    ...
    if (
      ...
      if (
        ...
        errors.push(
          `${id}: scalingTriggers must be an array`,
        );
      } else {
        for (
          const trigger of
          ...
          if (
            !SCALING_TRIGGER_MECHANICS.has(
              trigger,
            )
          ) {
            errors.push(
              `${id}: invalid scaling trigger "${trigger}"`,
            );
          }
        }
  ...
  if (!mechanics) {
    errors.push(
      `${id}: missing mechanics`,
    );
    continue;
  }

  if (!Array.isArray(mechanics.provides)) {
    errors.push(
      `${id}: mechanics.provides must be an array`,
    );
  } else {
    for (const supply of mechanics.provides) {
      if (
        !MECHANIC_SIGNALS.has(
          supply.signal,
        )
      ) {
        errors.push(
  ...
  if (!Array.isArray(mechanics.consumes)) {
    errors.push(
      `${id}: mechanics.consumes must be an array`,
    );
  } else {
